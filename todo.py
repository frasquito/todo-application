...
class TodoApp(ft.UserControl):
    def build(self):
        self.new_task = ft.TextField(
            hint_text="¿Qué se debe hacer?", on_submit=self.add_clicked, expand=True
        )
        ...
        self.items_left = ft.Text("quedan 0 items")
    ...
    async def update_async(self):
        ...
        for task in self.tasks.controls:
            task.visible = (
                status == "todas"
                or (status == "activa" and task.completed == False)
                or (status == "completada" and task.completed)
            )
            if not task.completed:
                count += 1
        self.items_left.value = f"quedan {count} item(s) activos"
        await super().update_async()


async def main(page: ft.Page):
    page.title = "Nuestra Aplicación de Tareas"
